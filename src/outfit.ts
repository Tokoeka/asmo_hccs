...
export default function uniform(...changes: (Item | [Item, Slot])[]): void {
    const defaultUniform = {
        ...
        shirt: $items`LOV Eardigan, fresh coat of paint`,
        pants: get("sweat", 0) < 100
...
export const itemOutfit = new OutfitPlan(
    {
        ...
        offhand: $item`unbreakable umbrella, cursed magnifying glass`,
...
export const hotresOutfit = new OutfitPlan(
    {
		back: $item`unwrapped knock-off retro superhero cape`,
        weapon: $item`industrial fire extinguisher, Fourth of May Cosplay Saber`,
        offhand: $item`meteorite guard, latte lovers member's mug`,
        pants: $item`designer sweatpants`,
...
const familiarAndEquip =
		have($familiar`Doppelshifter`) && !inHardcore()
			...
			: {
					...
					equip: have($item`astral pet sweater`)
...
export const weaponOutfit = new OutfitPlan(
    {
        hat: $items`extra-wide head candle, seal-skull helmet`,
